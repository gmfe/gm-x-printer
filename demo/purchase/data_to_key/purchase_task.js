...
function generateTable(tasks) {
  return tasks.map((task, index) => {
    ...
    if (Number(task.stock) < 0) {
      suggestPurchase = `${Big(task.plan_purchase_amount).toFixed(2)}${
        task.std_unit_name
      }`
    } else {
      suggestPurchase =
    ...
    return {
      ...
      [i18next.t('参考成本')]: `${
        task.price ? money(task.price) : 0
      }${Price.getUnit() + '/'}${task.std_unit_name}`,
      [i18next.t(
        '规格'
      )]: `${task.sale_ratio}${task.std_unit_name}/${task.sale_unit_name}`,
      ...
      [i18next.t('计划采购_采购单位')]: Big(task.plan_purchase_amount)
        .div(task.sale_ratio)
        .toFixed(2),
      ...
      [i18next.t('预采购金额')]: money(
        Big(task.plan_purchase_amount).times(task.price || 0)
      ),
      [i18next.t('采购金额')]: '',
      ...
      __details: task.addresses.map(o => {
        return {
          ...
          [i18next.t('采购数量_采购单位')]: Big(o.plan_purchase_amount)
            .div(task.sale_ratio)
            .toFixed(2),
...
function purchaseTask(data) {
  ...
  const common = {
    ...
    [i18next.t('采购员')]: purchaser.length
  ...
  // 按一级分类分组
  const groupByCategory1 = _.groupBy(normalTable, v => v._origin.category1_name)
  let kCategory = []
  let index = 1
